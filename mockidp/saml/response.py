...
def create_auth_response(config, session, saml_request):
    rendered_response = render_response(session, session.user, saml_request)
...
def render_response(session, user, saml_request):
